...
namespace TradeCaretaker.Tests
{
    public class RSITests
    {
        ...
        public void Calculate_Success(decimal[] inputStream, int periods)
        {
            ...
            for (var i = 0; i < trades.Length - periods; i++) rsi.Calculate(trades.Slice(i, periods + 1));
        ...
        {
            var periods = 2;
            var inputStream = new decimal[4] {4, 3, 2, 1};
            var trades = inputStream.AsTradesWitRates();
        ...
        {
            var periods = 2;
            var inputStream = new decimal[4] {1, 2, 3, 4};
            var trades = inputStream.AsTradesWitRates();
